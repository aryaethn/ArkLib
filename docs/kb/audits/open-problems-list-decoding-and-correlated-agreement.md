...
| `D2.20` | Extension code `C_F` | present-but-incomplete | [ExtensionCodes.lean](../../../ArkLib/Data/CodingTheory/ExtensionCodes.lean) | `CodingTheory.extensionCode` | Set-level definition; uses coordinate-projections `P.coord j` of D2.19. **B-side closure proven** (`extensionCode_add_mem`, `extensionCode_psi_smul_mem`); **F-scalar closure (`extensionCode_smul_mem`)** still a tagged sorry, but post-Algebra/Basis refactor of D2.19 the obstruction is now mechanical (routine `Finset.sum` shuffle via `Basis.sum_equivFun`) rather than structural ("needs F-algebra structure constants" — those are now `P.coord j (α * P.basis m)`, directly computable). Distance equality `δ_min(C_F) = δ_min(C_B)` from DP25 not formalised. |
