...
| `C6.2` | Construction 6.2 | present | `ToyProblem.Spec.pSpec`, `Statement`, `OracleStatement`, `Witness`, `accepts`, `inputRelation`, `outputRelation`, `outputRelation₁`, `prover`, `verifier`, `reduction`, `oracleProver`, `oracleVerifier`, `oracleReduction`, `accepts_of_inputRelation` in [Spec/General.lean](../../../ArkLib/ProofSystem/ToyProblem/Spec/General.lean) | same | Three-round `ProtocolSpec` (γ / g / spot-checks) with `OracleInterface` / `SampleableType` instances. Full honest `Prover` / `Verifier` / `Reduction` triple (computable, non-oracle) **and** `OracleProver` / `OracleVerifier` / `OracleReduction` flavour (verify body still a stub pending `OracleSpec.query` plumbing). Honest-completeness point form `accepts_of_inputRelation` **proven** (ring + linearity). IRS instantiation in `Impl/IRS.lean`. |
