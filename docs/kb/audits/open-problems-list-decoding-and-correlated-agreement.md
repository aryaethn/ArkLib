...
| `L2.10` | `\|Λ(C^≡m,δ)\| ≤ binom(b+r,r)·\|Λ\|^r` | present-but-incomplete | `InterleavedCode.lambda_le` in [InterleavedCode.lean](../../../ArkLib/Data/CodingTheory/InterleavedCode.lean) | same | External admit `[GGR11]`. Statement binds `η := δ_C − δ`, `b := ⌈δ/η⌉`, `r := ⌈log₂(δ_C/η)⌉` and shows `|Λ(C^{≡m}, δ)| ≤ (b+r choose r) · |Λ(C, δ)|^r` for all `m ≥ 1`. |
