...
| `L6.12` | List-decoding lower-bound attack | present-but-incomplete | `ToyProblem.simplified_iop_soundness_listDecoding_lb` in [SoundnessBounds.lean](../../../ArkLib/ProofSystem/ToyProblem/SoundnessBounds.lean) | same | **`paper-proof-owed`** (ABF26's OWN §6.4.1 result). **Statement corrected 2026-06-04 (finding S5):** conclusion now `\|Ω\|.ncard ≥ N·\|F\|/(\|F\|+2N)` (the canonical `.tex` final bound `N/(\|F\|+2N)`), not the old `N·\|F\|/(\|F\|+N−1)` (which was the *intermediate* `\|S_v\|` bound and overshot). Now also carries `[Nonempty ι]`, a linearity hypothesis (`∃ enc, range enc = C`), and certifies the violation (`¬ R̃²` conjunct). **Proof DEFERRED to Phase 4** — needs Λ-enumeration (`Fin N → message-pairs`), an inner-product collision-probability lemma over `F^k`, *two* Claim-B.1 applications, affine-map injectivity, and denominator algebra. Sorry on the proof only. |
