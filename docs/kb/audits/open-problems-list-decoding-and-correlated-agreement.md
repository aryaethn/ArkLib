...
| `C6.2` | Construction 6.2 | present | `ToyProblem.Spec.pSpec`, `Statement`, `OracleStatement`, `Witness`, `accepts`, `inputRelation`, `outputRelation`, `prover`, `verifier`, `reduction`, `accepts_of_inputRelation` in [Spec/General.lean](../../../ArkLib/ProofSystem/ToyProblem/Spec/General.lean) | same | Three-round `ProtocolSpec` (γ / g / spot-checks) with `OracleInterface` instances; IOR-shaped `inputRelation` / `outputRelation`; the §6.1 `accepts` decision predicate; and the honest `Prover` / `Verifier` / `Reduction` triple (with the oracle codewords threaded through the bundled `StmtIn`, matching the `OracleReduction.toReduction` shape). Honest-completeness point form `accepts_of_inputRelation` is a tagged sorry pending the linear-algebra bookkeeping. |
...
| `L6.6` | Knowledge soundness of Construction 6.2 | present-but-incomplete | `ToyProblem.Spec.protocol62_knowledgeSound` in [Spec/General.lean](../../../ArkLib/ProofSystem/ToyProblem/Spec/General.lean) | same | Placeholder statement; full Reduction-level formulation lands once the prover object is wired up. |
...
| `L6.8` | Round-by-round knowledge soundness of Construction 6.2 | present-but-incomplete | `ToyProblem.Spec.protocol62_rbrKnowledgeSound` in [Spec/General.lean](../../../ArkLib/ProofSystem/ToyProblem/Spec/General.lean) | same | ArkLib's `Verifier.rbrKnowledgeSoundness` + `KnowledgeStateFunction` (matching paper Def A.5) is in `OracleReduction/Security/RoundByRound.lean`. Wiring is deferred. |
