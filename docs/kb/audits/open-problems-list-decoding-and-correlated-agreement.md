...
| ABF26 ID | Paper item | Status | Lean refs | Lean target | Notes |
| --- | --- | --- | --- | --- | --- |
| `D4.1` | `ε_ca(C,δ_fld,δ_int)` | present | `ProximityGap.epsCA`, `ProximityGap.epsCA'`, `epsCA_mono_δ_fld`, `epsCA_antitone_δ_int`, bridge `δ_ε_correlatedAgreementAffineLines_iff_epsCA_le` in [EpsilonErrors.lean](../../../ArkLib/Data/CodingTheory/ProximityGap/EpsilonErrors.lean); coexisting predicate API `δ_ε_correlatedAgreement*` in [Basic.lean](../../../ArkLib/Data/CodingTheory/ProximityGap/Basic.lean) | existing | Definition + monotonicity in both arguments + `AffineLines` bridge all closed. Bridges to `Curves` and `AffineSpaces` predicates remain as follow-ups. |
